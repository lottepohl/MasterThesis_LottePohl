...
knitr::kable(x = tagged_animal_info %>% 
               ...
               mutate(release_loc = ifelse(release_loc == "Neeltje Jans", 1, 2),
                      tag_serial_number = cell_spec(tag_serial_number, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      sex = cell_spec(sex, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      length1 = cell_spec(length1, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      weight = cell_spec(weight, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      release_date_time = cell_spec(release_date_time, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      n_detect = cell_spec(n_detect, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      days_detected = cell_spec(days_detected, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      hours_detected = cell_spec(hours_detected, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      date_last_detected = cell_spec(date_last_detected, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      days_at_liberty = cell_spec(days_at_liberty, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      residency_index = cell_spec(residency_index, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F)),
                      
                      release_loc = cell_spec(release_loc, bold = ifelse(tag_serial_number %in% dst_summary$tag_serial_number, T, F))
                      ),
             booktabs = T, escape = F,
