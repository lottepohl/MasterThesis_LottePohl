...
#| out-width: 90%
#| tbl-cap-location: top
#| tbl-cap: "Information on the recovered tags. The death date was visually determined from the raw dephlog and marks the date after which only a tidal signal remains in the depth timeseries. The time at liberty is the time period between the release and death date. The tag recapture date was not recorded for tags 1293321, 1293308 and 1293304."
...
River. @fig-mapstudyarea-3 displays an overview of the acoustic receivers in the estuary of the Western Scheldt. A detection probability of \>50 % was estimated for a radius of 500-700 m around the receiver [@goossens_2022], and \>70
...
#| - "Acoustic receivers of the Permanent Belgian Acoustic Receiver Network (PBARN)."
#| fig-pos: "b"
...
Detections were accessed from the ETN database (https://www.lifewatch.be/etn/)[https://www.lifewatch.be/etn/] using the package `etn` [@etn]. Tags with only a single
...
Marine Data Archive (MDA, dataset DOI: (https://doi.org/10.14284/605)[https://doi.org/10.14284/605]). The temperature log
...
`r round((detections_tempdepth_daynight %>% dplyr::filter(station_name == "ws-OG10") %>% nrow()) / (detections_tempdepth_daynight %>% dplyr::filter(area == "WS1") %>% nrow()) * 100, digits = 2)`% are at station OG10 (see @fig-mapstudyarea-3).
<!-- Tagging of 30 *M. asterias* individuals occurred in July and August, 2018 -->
<!-- (`r tagging_info$date2[1]`, n = `r tagging_info$n[1]`, -->
<!-- `r tagging_info$date2[2]` and 03, n = `r tagging_info$n[2]` and n = -->
<!-- `r tagging_info$n[3]`) and in July, 2019 (`r tagging_info$date2[4]` and -->
...
<!-- To test the first hypothesis, the residency indices among sexes were tested for normality (Shapiro-Wilk normality test, $\alpha$ = 0.05, *p~females~* = `r hyp1_results %>% dplyr::filter(test_aim == "normality_f") %>% dplyr::select(p_val) %>% dplyr::pull() %>% round(digits = 4)`, *p~males~* = `r hyp1_results %>% dplyr::filter(test_aim == "normality_m") %>% dplyr::select(p_val) %>% dplyr::pull() %>% round(digits = 4)`), and since *p~females~* < 0.05 it has to be assumed that the residency indices of the females do not follow a normal distribution. -->
<!-- <!-- and since both *p*-values are above the significance level of 0.05, it has to be assumed that the residency indices do not follow a normal distribution.  --> -->
<!-- Due to high robustness of Welch's t-test regarding non-normality [@rasch_2004], however, this method was preferred over its non-parametric counterpart, the Wilcoxon rank-sum test. A one-sided Welch two-sample t-test with unequal variances was conducted ($\alpha$ = 0.05, *p* = `r hyp1_results %>% dplyr::filter(test_aim == "equality_of_means") %>% dplyr::select(p_val) %>% dplyr::pull() %>% round(digits = 4)`), showing that the residency index of females is significantly higher than the residency index of males. -->
...
#| out-width: 100%
#| fig-height: 3
#| fig-cap: "Heatmap of the acoustic detections per acoustic receiver of female *M. asterias* in the Western Scheldt (receiver arrays *WS1* and *WS2*)."
#| fig-pos: "H"
...
The acoustic detections of an individual at a day (`r round(detections_OG102019$n_detect %>% mean(), digits = 2)` $\pm$ `r round(detections_OG102019$n_detect %>% sd(), digits = 2)`, median = `r detections_OG102019$n_detect %>% median()`) are maximal for tag 299 with n = 232 detections on October 13, 2019. Tags 313, 308, 299 and 297 show a cluster of about 14 - 21 days with consecutive detections. Tag 308 has two clusters of subsequent detections, between May 13 and June 09, and between July 16 and August 12, 2019.
...
The full depthlog of tag 308 (female, @fig-dst308-1), comprising 366 days, shows depth between 0 and 20 m from the time of tagging in August, 2018, to the beginning of October. Then the depth gets deeper over a period of about two weeks, and between mid-November and mid-December the sensor recorded depths between 50 and 75 m. From December, 2018 until March, 2019, the depth ranges between 75 m and close to the water surface. From mid-March on, the depth remains shallower than 60 m. From May to June, 2019, depth is relatively constant around 17 m, from mid-June to mid-July it is relatively constant around 10 m and until the sensor stops recording the depth goes back to just under 20 m.
...
The subset in @fig-dst308-3 shows a period of a month between February and March, 2019. Here, periods of smaller depth ranges from 40 to 60 m (between February 11 and 18 and between February 25 and March 3) can be observed with periods of larger depth ranges from 5 to 75 m (between February 18 and 24) in between, can be observed.
...
The example depthlog of September, 2018 in @fig-dst321-2 shows a depth range between 10 and 15 m during daytime (a sinusoidal pattern similar to the one in @fig-dst308-2), and minimum depths of around 2 m during night time.
...
@fig-acf. An autocorrelation value of -1 refers to perfect anti-correlation, 1
referring to perfect correlation, and 0 refers to no correlation at all. Autocorrelation is above
...
The two lines of research encompass assessing seasonality and sex-specificity regarding the presence of *M. asterias* in the BPNS and the Western Scheldt, and its depth utilisation. In the following sections the chosen analyses and their results with respect to the research questions stated in @sec-intro-aims will be interpreted and discussed. @sec-disc-seasonalpresencefemales will cover the first line of research (presence of *M. asterias* in the BPNS and Western Scheldt), and @sec-disc-dst-movementpatterns will cover the second line of research (depth utilisation). In addition, the potential of ADST for research on *M. asterias* will be discussed (@sec-disc-adst), as well as limits of the data and telemetry methods (@sec-disc-limits) and the chosen analyses (@sec-disc-reflection).
 ...
 <!-- - write a few introductory sentences about what happened in the study and during the analyses. -->

<!-- ## Acoustic Detections -->

## Presence of *M. asterias* in the Western Scheldt and BPNS (Research Line 1) {#sec-disc-seasonalpresencefemales}

This section discusses the results of the acoustic detections (see @sec-res-acoustic) in the context of the three research questions research line 1. Those which involve seasonal patterns (RQ 1), differences between sexes (RQ 2), and spatial preferences (RQ 3) within the BPNS and the Western Scheldt.
...
There are no acoustic detections at all between November and April in all years with acoustic detections (2018-2020, @fig-abacus). This suggests a seasonal presence of *M. asterias* in the Western Scheldt between April and November.
...
Not more than one male was ever detected in a month. Three males were detected in the BPNS (one each on August, 2018, and in April and May, 2019), and one male was detected in the WS1 array in April, 2020. Three of those four individuals were detected in the year following their release, indicating philopatric behaviour for males. @griffiths_2020 reported direct philopatry for a female *M. asterias*, and @breve_2016 suggested philopatry for the species based on tag and recapture data. The return of the three males (tags 311, 315 and 316) into the BPNS and the WS1 area could be the first direct reporting of philopatry for male *M. asterias*.
...
The periods of 10 to 20 days of subsequent detections of females (especially tags 299, 308 and 313) at station OG10 suggest partial residency of these individuals. Uncovering potential reasons for this behaviour and 
A relevant aspect to study in the future would be to explore reasons of this behaviour and whether the females could be giving birth during their residential clusters at station OG10.
...
The Scheldt estuary can be considered a suitable pupping habitat for *M. asterias* due to high nutrient levels and food availability of the Continental coastal water [which enters the estuary through tidal currents, @wolff_1973], and monthly water temperatures above 15 °C between June and September [@speed_2010]. The fact that few sharks were detected upstream past Flushing could be due to low salinities, or to anthropogenic influences like dredging. This, however, should be further investigated in follow-up studies.
...
During April 2019, the male shark does not show any prevailing periodicity patterns in the wavelet scalogram at hourly scale (@fig-waveletresults321-). The month of May 2019, however, shows prevalent periodicities between 12 and 64 h, potentially indicating migration behaviour. There is decreased vertical activity compared to the winter months from mid-May onwards for the female and from June on for the male, indicating that the individuals might have arrived at their summer habitat by then.
...
## Limits and biases of the telemetry methods used {#sec-disc-limits}
...
To conclude, the findings related to the research questions stated in the introduction (see @sec-intro-aims) will be presented.
...
available in the Integrated Marine Information System (IMIS) with the Digital Object Identifier (https://doi.org/10.14284/605)[https://doi.org/10.14284/605].
...
@fig-dsttempannex shows the raw temperature logs of tags 308 and 321. Tag 308 experienced the maximum temperature of 22.9°C on August 06, 2018 and its minimum temperature of 8.65°C on March 23, 2019.
...
## **Annex B.** Fast Fourier Transformation {-}
