...
These models use the seafloor bathymetry in combination with other environmental variables such as temperature to estimate the most likely path that was taken by the animal (*ref*). 
...
It is the only occurring *Mustelus* species in the Northeast Atlantic (*ref*).
***Elaborate***
...
with females moving into the English Channel and the Bay of Biscay, and males moving into the Northern North Sea and into the English Channel [@breve_2020]. Potential reasons of this sex-specific migration are females avoiding mating, resource competition, or reduction of embryo mortality [@speed_2010].
...
#| out-width: 100%
#| tbl-cap-location: top
#| tbl-cap: "Information on the tagged individuals. Release location 1 refers to Neeltje Jans, release location 2 refers to just outside of the Western Scheldt. ***Include Residency Index per individual?***."
...
The acoustic detections of the remaining tags over time are shown in @fig-abacus.
...
#| out-width: 95%
#| fig-height: 3
#| fig-cap: "Acoustic detectiosn of the tagged starry smooth-hounds, the colour responds to the area the receiver was in. The black cross refers to the tagging date."
#| fig-pos: "H"
...
Out of the `r close_stations %>% filter(area %in% c("WS1", "WS2")) %>% dplyr::select(station_name) %>% unique() %>% nrow()` receiver stations overall in the WS1 and the WS2 receiver array, *M. asterias* individuals were detected at `r summary_all2$station_name %>% unique() %>% length()` receiver stations.
...
The depth logs of tag 295, 308 and 321 (the two long term DST) are shown in @fig-dst. 
<!-- The depth logs of the two long term DST are shown in @fig-dst308 and @fig-dst321.  -->
The remaining depth logs are depicted in @sec-annexdst_raw. Only evert fifth value is depicted to increase the readability of the plot, making the time interval between two values 10 minutes.
...
The resulting periodograms from the Fast Fourier Transform for tags 295 (*m*), 308 (*f*) and 321 (*m*) are depicted in @fig-fft.
...
#| fig-height: 1.5
#| layout-ncol: 1
#| fig-cap: "Periodogram from three recovered tags."
#| fig-subcap: 
#| - "Tag 295 (m), 31 days of liberty"
...
#| fig-height: 2.5
#| layout-ncol: 1
#| fig-cap: "Daily median depth, maximum and minimum depth in the ribbon. Dial vertical migration behaviour as coloured bars in the background."
#| fig-subcap: 
#| - "Tag 308 (f)"
#| - "Tag 321 (m)"
#| fig-pos: "H"
...
#| out-width: 95%
#| fig-height: 2.5
#| fig-cap: "Daily median depth, and maximum and minimum depth in the ribbon. Dial vertical migration behaviour as coloured bars in the background."
#| fig-pos: "H"
...
### The Western Scheldt as a tagging ground for *M. asterias*
