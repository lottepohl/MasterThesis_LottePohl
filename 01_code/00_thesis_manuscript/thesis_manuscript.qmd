...
knitr::kable(abbreviations_list,
             booktabs = T, escape = F, format = "markdown") %>% kableExtra::kable_styling(position = "center",
...
This study employed ADST to characterise the migration and residency of M. asterias in the North Sea regarding seasonality and sex. Exploratory and spectral analyses were conducted on a July 2018 to July 2020 ADST dataset.
In 2018 and 2019, 30 adult M. asterias (19 females, 11 males) were equipped with ADST-V13TP (Innovasea, 518 days estimated battery life) in the Scheldt Estuary. The Permanent Belgian Receiver Network (PBARN) encompasses 160 active receivers distributed across the Belgian Part of the North Sea (BPNS) and the Scheldt Estuary. 14 females and 4 males were detected 10940 times by 27 acoustic receivers. 
All detections were between April and November, suggesting seasonal presence of M. asterias in the receiver area. 96% of detections were in the outer Western Scheldt, and 97% of detections were from females. This suggests the outer Western Scheldt to be a relevant habitat for females between April and November.
8 ADST were recovered, one female and one male logging data for over a year. Both tags logged depths between 0 and 20 m between June and October, and between 10 and 75 m between October and June. The changing depth range reflects the migration of the sharks into deeper waters during winter. Continuous Wavelet Transform (CWT) resolves periodic patterns in the time domain and displays differences in periodicities between seasons and individuals. The CWT results suggest that the female is resting during summer and feeding in a biweekly pattern in winter, and that the male feeds continuously.
...
Spectral analysis of the depth logs of two individuals logging for over a year indicates feeding behaviour of the male throughout most of the year. The female shark primarily rests on the bottom during summer and feeds in a biweekly rhythm during winter.
...
Archival telemetry uses tags or biologgers that record and locally store environmental variables such as water pressure and temperature.
<!-- Popup Satellite Archival Tags (PSAT) disattach from the -->
<!-- animal and float to the surface, transmitting recorded data to -->
<!-- satellites, while  -->
Data Storage Tags (DST) require the retrieval of the
...
predefined intervals. Per detection, one value of either temperature or water depth gets transmitted. 
...
To determine the approximate location (i.e., latitude and longitude), of a DST log (which represents the vertical dimension), stochastic geolocation models are commonly used [@gatti_2021]. These models involve bottom bathymetry and other environmental factors, including water temperature, to estimate the trajectory followed by the fish [@nielsen_2004].
<!-- To infer the latitude and longitude (i.e., the horizontal dimension) of a DST log (which is in a vertical dimension), stochastic geolocation models are used [@gatti_2021]. These models use the bottom bathymetry in -->
...
#| out-width: 90%
#| layout-ncol: 1
#| fig-cap: "Results of geolocation modelling (without behavioural states) on the raw DST depthlogs. The estimated trajectory of the male shark is shown in panel **a)**, the estimated trajectory of the female shark is shown in panel **b)**. Tagging locations are marked in yellow. "
#| fig-subcap:
...
embryo asynchronism). @breve_2016 and @griffiths_2020 reported philopatric behaviour
(i.e., females returning to the same area for parturition). Identified parturition grounds for *M. asterias* include the
estuary of the Eastern Scheldt and, more generally, the Bristol Channel, Southern North
...
bottom.
<!-- , indicating that it swims closely above the bottom during -->
...
<!-- - daily vertical migration: present (find source), but also (most likely) horizontal movement associated with the vertical movement, i.e. the animal stays on the bottom and swims shorewards during the night, feeding on crustaceans (?) -->
...
potentially avoids predation by resting on the bottom [@griffiths_2020].
<!-- ### Migration -->
...
<!-- 2.  During summer, sharks predominantly use the bottom section of the -->
<!--     space around a receiver -->

<!-- -   predominantly = xx% (or result of chi2?) -->
<!-- -   bottom section = the deepest 75% around a receiver -->
...
August 2018, Latitude: `r release_locations %>% dplyr::filter(release_loc == "Western Scheldt") %>% dplyr::select(lat) %>% dplyr::pull()`, Longitude: `r release_locations %>% dplyr::filter(release_loc == "Western Scheldt") %>% dplyr::select(lng) %>% dplyr::pull()`). The tagging locations are marked in @fig-mapWS.
...
#| out-width: 90%
#| tbl-cap-location: top
#| tbl-cap: "Information on the recovered tags. The death date was visually determined from the raw depthlog and marks the date after which only a tidal signal remains in the depth timeseries. The time at liberty is the time period between the release and death date. The tag recapture date was not recorded for tags 1293321, 1293308 and 1293304."
...
Dierproeven* (reference 2016.D-0041.004 and 2016.D-0041.008). All animal treatments were
...
The study area encompasses two scales, reflecting the two lines of research of this study. The first research line focuses on the Belgian Part of the North Sea (BPNS) and the estuary of the Western Scheldt. It also encompasses the tagging location *Neeltje Jans*, which is not part of the Western Scheldt or the BPNS but lies within the Eastern Scheldt.

The second research line includes year-round behaviour and thus, the study area is broader and less defined. Since the species is thought to move both into the English Channel and into the North Sea north of the BPNS, both regions constitute the study area for the second line of research. An overview on the different study areas is displayed in @fig-mapstudyarea.
...
The bottom depth averages around 11 m with occasional troughs of up to
...
Estuary is provided in @fig-mapstudyarea-2. Fish and crustacean species
...
The East Anglian coast lies within the Southern North Sea and is characterised by shallow coastal waters between 20 and 25 m water depth, fast tidal streams and sandy and muddy bottom sediments [@harrison_1990]. 
...
bottom depth increases from around 25 m in the east to 75 m in the
...
Bottom temperatures vary less in the western EC (range: 9°C to 14°C) than in
the eastern EC (8°C and 16°C). Species living on the
bottom include Crustacea like the edible crab (*Cancer pagurus*), the
...
#| out-width: 90%
#| layout-ncol: 1
#| fig-cap: "Overview maps of the study area. **a)** The Southern North Sea and the English Channel, **b)** The Belgian Part of the North Sea (BPNS) and the Scheldt Estuary. ES = Eastern Scheldt, WS = Western Scheldt. Each black circle marks one acoustic receiver station."
#| fig-subcap:
#| - "The Southern North Sea and the English Channel."
#| - "The Belgian Part of the North Sea (BPNS) and the Scheldt Estuary."
#| fig-pos: "H"
...
River. @fig-mapWS displays an overview of the acoustic receivers in the estuary of the Western Scheldt. A detection probability of \>50 % was estimated for a radius of 500-700 m around the receiver [@goossens_2022], and \>70
...
Marine Data Archive (MDA, dataset DOI: (https://doi.org/10.14284/605)), and are the same dataset as the *M. asterias* data from @goossens_2023. The temperature log
...
filter order *p = 1*, filter length *n = 5*] was applied on all summaries (daily, daytime and nighttime) using the
...
#### Fast Fourier Transformation (FFT) {#sec-mm-fft}
...
#### Continuous Wavelet Transform (CWT) {#sec-mm-cwt}
...
<!-- both the raw depthlog data and the median depth per date, resulting in hourly and daily resolution of periodicities, respectively. The wavelet scalograms of the raw depthlog display the maximum power per day in the scalogram. -->


<!-- Next, Continuous Wavelet Transform (CWT, a specific type of Wavelet Analysis) was performed on the daily summary -->
<!-- statistics of the long-term DST depthlogs, using the `wt()`-function from the -->
...
<!-- resolution loss, frequencies were compared against white noise using a $\chi^2$ test [@grinsted_2004]. CWT was performed on both the raw depthlog data and the median depth per date, resulting in hourly and daily resolution of periodicities, respectively. The wavelet scalograms of the raw depthlog display the maximum power per day in the scalogram. -->
...
## Acoustic detections {#sec-res-acoustic}

Two tags only had a single detection (serial numbers
...
#| out-width: 100%
#| fig-height: 3
#| fig-cap: "Heatmap of the acoustic detections per acoustic receiver of female *M. asterias* in the Western Scheldt (receiver arrays *WS1* and *WS2*). The stations are ordered per receiver array, and from north to south."
#| fig-pos: "H"
...
The acoustic detections of an individual at a day (median: `r detections_OG102019$n_detect %>% median()`) spread from `r detections_OG102019$n_detect %>% min()` to `r detections_OG102019$n_detect %>% max()` detections for tag 299 on October 13, 2019. 

Tags 313, 308, 299 and 297 show a cluster of about 14 - 21 days with consecutive detections. Tag 308 has two clusters of subsequent detections, between May 13 and June 09, and between July 16 and August 12, 2019.
...
#| out-width: 90%
#| fig-height: 3
#| fig-cap: "Heatmap of the acoustic detections in 2019 of female *M. asterias* individuals at receiver station OG10. The Residency Index (RI, i.e. the number of days detected divided by the period between the first and last detection among all individuals at station OG10 in 2019) is displayed next to the tag serial number."
#| fig-pos: "H"
...
DST) are shown in @fig-dst308-1 and @fig-dst321-1, respectively. Example subsets from summer and winter are also shown.

The full depthlog of tag 308 (female, @fig-dst308-1), comprising 366 days, shows depths between 0 and 20 m from the time of tagging in August, 2018, to the beginning of October. Then the depth gets deeper over a period of about two weeks, and between mid-November and mid-December the sensor recorded depths between 50 and 75 m. From December, 2018 until March, 2019, the depth ranges between 75 m and close to the water surface. From mid-March on, the depth remains shallower than 60 m. From May to June, 2019, depth is relatively constant around 17 m, from mid-June to mid-July it is relatively constant around 10 m and until the sensor stops recording the depth goes back to just under 20 m.
@fig-dst308-2 shows an example subset of the depthlog of tag 308 in September, 2018. Roughly, the depth alternates between 12  - 14 and 16 - 20 m in a sinusoidal pattern with a wavelength of about 12 hours.
@fig-dst308-3 shows the recorded depth between February and March, 2019. Here, periods of smaller depth ranges from 40 to 60 m (between February 11 and 18 and between February 25 and March 3) can be observed with periods of larger depth ranges from 5 to 75 m (between February 18 and 24) in between.


The full depthlog of tag 321 (male, @fig-dst321-1) is 485 days long and shows depth ranges between 0 and 30 m from tagging until the end of October, 2018. Then, depth gets deeper over a period of roughly 10 days, stays at 35 to 50 m until December, 2018 and then ranges from 5 m to 50 m until March, 2019. Until May, 2019 depth ranges between 10 and 35 m, then the sensor recorded depth values of 60 m on May 03, 2019. After that, depth gets shallower with ranges from 0 to mostly 20 m until mid-September. Until the end of the depthlog November 15, 2019, depth gets deeper to around 35 m and then to around 40 m.
The example depthlog of September, 2018 in @fig-dst321-2 shows a depth range between 10 and 15 m during daytime (a sinusoidal pattern similar to the one in @fig-dst308-2), and minimum depths of around 2 m during nighttime.
...
#| out-width: 120%
#| fig-height: 2.5
#| fig-cap: "Data from the recovered tag 321. This male shark was tagged on Juy 19, 2018."
#| fig-subcap: 
#| - "Depthlog, subsampled to one value every 30 minutes."
#| - "Example subset from September 2018."
#| - "Example subset from February and March 2019."
...
#| fig-height: 1.5
#| layout-nrow: 2
#| fig-cap: "Autocorrelograms of daily median depth. Numbers and dots in blue display the days at which autocorrelation values are 0, minimum and maximum."
#| fig-subcap: 
#| - "Tag 308 (f)"
#| - "Tag 321 (m)"
#| fig-pos: "H"
...
filtered daily median depth. The resulting wavelet scalograms for the two long-term DST depthlogs are shown below, in @fig-waveletresults308
...
The depthlog of tag 308 (f) shows periodic cycles of approximately 14 and 30 days in October, 2018 and between December and June, 2019 (@fig-waveletresults308-2).
...
The depthlog of tag 321 (male) shows punctually high periodicities between 4 to 64 hours at the beginning and end of October, 2018, throughout May, 2019, and at the beginning of October, 2019 (@fig-waveletresults321-1). Significant periodicities are sporadically present during these periods, between roughly 4 and 12 hours. A period band of 24 hours is present August to October, 2018, January and February, 2019 and between July and October, 2019. A period band of 12 hours is present to a lesser extent during the same periods.
...
@fig-adst308 shows the raw depthlog of tag 308 (f), superposed with the acoustic detections of
...
The following section will critically reflect on the datasets used, the analysis methods, and interpret the findings in the context of already existing knowledge on *M. asterias*.
...
Thirdly, the results of this work will be interpreted regarding present knowledge on movement ecology of *M. asterias*. The two research lines of this study encompass assessing seasonality and sex-specificity regarding the presence of *M. asterias* in the BPNS and the Western Scheldt, as well as its depth utilisation. @sec-disc-seasonalpresencefemales will cover the first line of research (presence of *M. asterias* in the BPNS and Western Scheldt), and @sec-disc-dst-movementpatterns will cover the second line of research (depth utilisation) and the five research questions stated in @sec-intro-aims.
This section will finally discuss the potential of ADST as a novel tag technology for research on *M. asterias* (@sec-disc-adst) and the gained ecological knowledge on the species in the context of related *Mustelus* species (@sec-disc-otherspecies).
...
Other possible factors include insufficient asepsis of surgery tools and the tag itself, or negative physiological responses due to handling [@rub_2014]. This includes too much time outside of the water, injuries from the fishing method (hook and line, in this case), or the tag implantation taking too long.
...
Death due to predation by an endothermic mammal predator such as the grey seal can be excluded, because the temperature did not substantially rise [to around 38°C, @austin_2006] prior to the animals’ deaths (all raw depth an temperature logs are shown in @fig-dsttempannex. Generally, the temperature logs of the short-term DST appear erroneous, note for example that the temperature remains 21.2°C for tag 319 at all times between August 4 and 12, 2018. The possibility of predation by an ectothermic predator cannot be ruled out at this point, since temperature cannot provide any relevant insights but instead the depthlog would show extraordinary vertical movement [see @seitz_2019 for examples of predation by endothermic versus ectothermic predators]. The possibility of predation by an ectothermic animal such as the blue shark *Prionace glauca* could be a next investigative step.
...
Essentially, two types of dataset were used for study: Firstly, the acoustic detections of the tagged sharks, and secondly, the DST logs from recovered tags.
...
While autocorrelation might not be the most conclusive exploratory analysis, it can be a useful auxiliary tool to explore the depth time series data and potentially confirm identified periods with occurring behavioural switches. 
...
This section discusses the results of the acoustic detections (see @sec-res-acoustic) in the context of the three research questions research line 1. Those involve seasonal patterns (RQ 1), differences between sexes (RQ 2), and spatial preferences (RQ 3) within the BPNS and the Western Scheldt.
...
There are no acoustic detections at all between November and April in all years with acoustic detections (2018-2020, @fig-abacus). A tagged animal is detected with an estimated probability of >70 % within a 200 m radius around the acoustic receiver [@reubens_2018]. Thus, no detections do not imply that no tagged animal was present, but the consistency of no detections between November and April throughout the whole study period suggests a seasonal presence of *M. asterias* in the Western Scheldt between April and November, which goes in line with assumptions on the residency and migration behaviour of the species [@breve_2016; @breve_2020; @griffiths_2020].
Most females (n = 7) were detected in the months of July, August and September, in the WS1 array.  This suggests that these months are especially relevant for *M. asterias* females in the Scheldt estuary.
...
97% of detections are of tags implanted into female *M. asterias*, and despite the higher number of females tagged (19 tagged females in contrast to 11 tagged males), this still suggests higher presence and habitat use of females in the receiver area compared to males.
Furthermore, not more than one male was ever detected in a month. Three males were detected in the BPNS (one each on August, 2018, and in April and May, 2019), and one male was detected in the WS1 array in April, 2020. Three of those four individuals were detected in the year following their release, indicating philopatric behaviour for males. @breve_2016 and @griffiths_2020 reported philopatry for *M. asterias* based on electronic, and tag/recapture data. The return of the three males (tags 311, 315 and 316) into the BPNS and the WS1 area could be the first direct reporting of philopatry for male *M. asterias*.
...
According to the acoustic detections, a hotspot of seasonal presence of mature *M. asterias* females is the area around the receiver OG10 (`r round((detections_tempdepth_daynight %>% dplyr::filter(station_name == "ws-OG10") %>% nrow()) / (detections_tempdepth_daynight %>% nrow()) * 100, digits =0)`
% of detections, and 10 out of 14 detected females, see @fig-heatmapOG10). According to fishers, the area offshore from Zoetelande (north of Dishoek) is a *M. asterias* fishing hotspot, so the coastal waters between Diskoek and Zoetelande are suggested to be important for the species. Whether or not the potential aggregations of mature females is related to parturition should be subject of further studies. There are several indications, however, that point towards the Western Scheldt and/or its surrounding waters to be a pupping area for *M. asterias*. The Eastern Scheldt is a confirmed
pupping ground [@breve_2016], and during the tagging for this study in
...
Females were either detected during many days (20 - 40) or few (5 - 10) days during the summer of 2019 (see @fig-heatmapOG10). This could indicate different behaviourial states between the females with many and few acoustically detected days, further supported by the large range of Residency Indices which are shown in the figure (the minimum RI of `r OG10_2019_RI$RI %>% min() %>% round(digits = 3)` differs from the maximum RI of `r OG10_2019_RI$RI %>% max() %>% round(digits = 3)` by a factor of close to 50).
...
The following section discusses the analysis results of the two long-term DST logs (of tag 308, female, and tag 321, male) in the light of the second research line, which entails the depth utilisation of *M. asterias*. The two research questions asked involve seasonal patterns (RQ 4) and differences in depth use between sexes (RQ 5).
...
In general, owing to the demersal lifestyle of *M. asterias*, shallower depths in the depthlog indicate a shoreward movement along the bottom, as opposed to a vertical movement into the water column. If, for example, the depthlog shows shallower depths during the night (as can be seen in  @fig-dst321-2) this suggests a movement into shallower, coastal waters to feed.
...
## Gained ecological knowledge on *M. asterias* in the context of *M. asterias* and related species {#sec-disc-otherspecies}
...
The code developed to conduct the analyses, make all visualisations (both figures and maps), and to write the manuscript are available on (https://github.com/lottepohl/MasterThesis_LottePohl)[https://github.com/lottepohl/MasterThesis_LottePohl].
