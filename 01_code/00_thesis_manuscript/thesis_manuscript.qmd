...
For the day/night summaries, sunrise and sunset times were extracted using the *'suncalc'*-package. On the daily summaries, a rolling mean with a window size of 3 days was applied to smooth the data.
On the daily summaries a Savitzky-Golay smoothing filter (filter order *p = 1*, filter length *n = 5*) was applied using the *sgolayfilt()*-function from the *signal* package. This particular filter smoothes the data but retains spikes in the data better than other smoothing filters like butterworth or averaging filters (like rolling mean) 
...
The following steps were only performed on the longterm DST logs.
...
RuLSIF was employed with the *'ts_detect()'*-function from the *'rulsif.ts'*-package [@brubaker_2021]. The algorithm can take an n-dimensional time series as input, as such allowing for more than one calculated summary statistic to be used as input. In this case, the daily median depth, maximum and minimum were chosen. These three parameters describe the daily depth use in a basic but overseeable way, making the interpretation of results easier.

Several hyperparameters needed to be chosen: the relative parameter $\alpha$, the window size *w*, the number of samples before and after a data point to compare between (*step*), and the threshold of the relative PE score above which a time point is classified as a change point (*thresh*). Hyperparameters are non-learned settings of a machine learning algorithm that control the learning behaviour during the algorithm's training (**ref**).
...
As default in the *'rulsif.ts'*-package, *step* is set to 10% of the amount of time points in the dataset used, but it is advised to decrease the step if computational resources allow for it (because small steps increase computation time). In the present study, RuLSIF was performed for different values of *step*: 2.5%, 5% and 10% of the input dataset length. For example, the DST 321 logged for 471 days, and a *step* of 5% of the input dataset length would amount to s *step* of 24 (days, rounded to the nearest integer).
...
Details on the tagged individuals is listed in @tbl-animalinfo.
...
#| out-width: 100%
#| tbl-cap-location: top
#| tbl-cap: "Information on the tagged individuals. # = times, h = hours, d = days detected. "

knitr::kable(tagged_animal_info) %>% 
  kableExtra::kable_styling(position = "center", latex_options = "hold_position")
```

*To Do: fix col width for table.*
