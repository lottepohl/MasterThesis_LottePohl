...
# tbl <- knitr::kable(abbrev)
# 
# # Use kable_styling() to suppress the caption
# tbl <- kable_styling(tbl, "striped", caption = "")
# 
# # Print the table
# print(tbl)
```
...
<!-- satellite telemetry,  -->
Telemetry allows for the quantification of animal migrations and (vertical and horizontal) habitat use [@hussey_2015], which informs species management and conservation spanning in marine, freshwater and terrestrial ecosystems [@beger_2010].
...
<!-- why I did CPD -->
It can be insightful, however, to gather inspiration from terrestrial behaviour segmentation methods. For the present study, a basic version of BCPA was applied to a vertical movement dataset from a demersal shark: Change Point Detection (CPD). CPD algorithms are designed to detect abrupt changes in signals. An abrupt change occurs very fast compared to the data sampling period, with somewhat constant properties before and after the change [@basseville_1993, pp.1-2]. CPD is a well-explored method in statistics which can also be employed on time series signals [@liu_2013].
...
*M. asterias* feeds on benthic and suprabenthic crustaceans, common prey includes (*insert species here*) [@mccullyphillips_2015; @mccullyphillips_2020]. 
...
@goossens_2023 used a geolocation model approach with a two-state HMM following an approach by @woillez_2016.

**put map with geolocation output by Jolien here?**
...
*is the study area the BPNS and the Scheldt, because there sharks were tagged? What are the greater north sea and the english channel for the study, as sharks putatively moved there during migration?*
...
::: {#fig-map}
![](C:/Users/lotte.pohl/Documents/github_repos/MasterThesis_LottePohl/01_code/00_thesis_manuscript/maps/map1_overview_manual.png)
...
The English Channel (EC) extends from the Dover Strait to England's southwestern tip, spanning 77 000 km^2 [@dauvin_2012]. The mean seafloor depth increases from around 25m in the east to 75m in the west [@dauvin_2012], except for the 174m deep *Hurd deep*, a trench located close to the Channel Island Guernsey.
