...
bathy_belgium <- read_csv(paste0(dir_path,"/00_data/environmental_layers/bathymetry/belgium/marmap_coord_1.9;50.9;4.7;52.4_res_0.25.csv"), show_col_types = F) %>% rename(longitude = V1, latitude = V2, depth_m = V3)
bathy_belgium_raster <- bathy_belgium %>% raster::rasterFromXYZ(crs = "EPSG:4326")
# raster::writeRaster(bathy_belgium_raster, paste0(dir_path), "/00_data/environmental_layers/bathymetry/bathy_NOAA_res_1.tif", format = "GTiff", overwrite = T)

bathy_belgium_coarse <- read_csv(paste0(dir_path,"/00_data/environmental_layers/bathymetry/belgium/marmap_coord_1.9;50.9;4.7;52.4_res_1.csv"), show_col_types = F) %>% rename(longitude = V1, latitude = V2, depth_m = V3)
bathy_belgium_coarse_raster <- bathy_belgium_coarse %>% raster::rasterFromXYZ(crs = "EPSG:4326")

bathy_northsea <- read_csv(paste0(dir_path,"/00_data/environmental_layers/bathymetry/north_sea/marmap_coord_-9;43;7;56_res_2.csv"), show_col_types = F) %>% rename(longitude = V1, latitude = V2, depth_m = V3)
