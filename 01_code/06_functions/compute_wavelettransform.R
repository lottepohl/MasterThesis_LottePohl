...
wavelet_output_compare <- function(dates, wt_output){
  ...
  wt_df <- wt_output$power %>% as.data.frame() %>%
    ...
    relocate(date, period, frequency) %>%
    left_join(signif, by = join_by(period, date), multiple = "all") %>%
    mutate(date = date, # %>% as.POSIXct()
           frequency_scaled = frequency %>% scale(),
           freq_norm = log2(frequency),
...
z <- wt_output$power %>% log2 #wt_output$signif

image(x = x, y = y, z = t(z), ylim = rev(range(y)))

plot.biwavelet(wt_output)
